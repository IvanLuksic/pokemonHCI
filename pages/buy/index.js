...
export default function Buy() {
  ...
  let [searchResult, setSearchResult] = useState(
    Array.from({ length: 11 }, () => Math.floor(Math.random() * 150 + 1))
  );
    ...
    <div className="container">
      <ContentContainer>
        ...
        >
          <PageHeading
            heading="Buy NFTs"
            subheading="Buy NFT Pokemon pictures"
            md={12}
            xs={11}
          />
          <Grid item md={12} xs={11}>
            <Search
              autocompleteList={PokemonList}
              searchType="pokemon"
              chipList={Object.getOwnPropertyNames(TypeToColorList)}
          ...
          <Grid item md={12} xs={11}>
            <PokeCardList
              ...
              isPokedex={false}
              setCartItems={setCartItemsB}
            />
          </Grid>
          {cartItemsB.length ? (
            <Cart cartItems={cartItemsB} setCartItems={setCartItemsB} />
          ) : null}

          {
            //Window.innerWidth se koristi zato što u protivnom mobile cart prekriva scroll to top fab
            cartItemsB.length && windowWidth < 920 ? (
              <MobileCart cartItems={cartItemsB} setCartItems={setCartItemsB} />
            ) : null
          }
        </Grid>
      </ContentContainer>
