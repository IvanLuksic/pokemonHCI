...
export default function AuthorCard({ name, surname, nickname, image, about }) {
    ...
    const fullName = name + (surname != null ? ( " " + surname) : "") + (nickname != undefined ? ( " - AKA " + nickname) : "")
    ...
    return (
        <Grid container direction="column" xs sx={{ width: "100%", height: "100%" }}>
            ...
            <Grid item xs sx={{ width: "100%", pb: "0.5em"}}>
                <Typography variant="h5" sx={{ fontWeight: 600}}>{fullName}</Typography>
            </Grid>
            <Grid item xs sx={{ width: "100%"}}>
                <Typography variant="h6" sx={{ fontWeight: 100}}>{about}</Typography>
            </Grid>
        </Grid>
    )
}
