...
export default function Search({ autocompleteList, setSearchResult, chipList, searchType, searchPool }) {
	...
	useEffect(() => {
		if (searchType == "pokemon") filterPokemon()
		else filterBlog()
	...
	function filterPokemon() {
		if (!searchValue && !chipValue) {
			...
			setSearchResult(
				autocompleteList
					.filter((value) => value.name.toUpperCase().includes(searchValue.toUpperCase()))
					.map((value) => value.id)
			)
		} else {
			setSearchResult(
				autocompleteList
					.filter((value) => value.type.includes(chipValue))
					.filter((value) => value.name.toUpperCase().includes(searchValue.toUpperCase()))
					.map((value) => value.id)
			)
		}
				...
				searchPool
					.filter(
						(value) =>
							...
							value.author.name ? value.author.name.toUpperCase().includes(searchValue.toUpperCase()) : null ||
							value.author.nickname ? value.author.nickname.toUpperCase().includes(searchValue.toUpperCase()) : null ||
							value.author.surname ? value.author.surname.toUpperCase().includes(searchValue.toUpperCase()) : null
					)
					.map((value) => value)
			)
		}
	...
	return (
		<Grid
			...
			alignItems="center"
			sx={{ width: "100%", mt: "1em", textAlign: "center" }}
		>
				...
				{chipList != null
					? chipList.map((value) => (
							<Chip
								label={value}
								key={value}
								color={chipValue === value ? "primary" : "default"}
